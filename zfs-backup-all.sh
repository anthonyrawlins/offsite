...
send_snapshot() {
    ...
    # Upload
    echo "    Uploading to ${remote_path}/${prefix}-${snap}.zfs.gz.age"
    rclone copy "$tmpfile" "${remote_path}/" --progress
    rclone moveto "${remote_path}/$(basename "$tmpfile")" "${remote_path}/${prefix}-${snap}.zfs.gz.age"
